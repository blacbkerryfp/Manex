...
function Home() {
  ...
  return (
    <div style={{
      ...
      height:'100vh', background:'linear-gradient(135deg,#0f172a,#1e293b)', color:'#fff',
      textAlign:'center', padding:'2rem'
    }}>
      <h1 style={{fontSize:'2.5rem', marginBottom:'1rem'}}>Bienvenue sur <span style={{color:'#a78bfa'}}>Manex</span></h1>
      <p style={{maxWidth:400, opacity:.8, marginBottom:'2rem'}}>Gérez facilement votre manuel d’exploitation en ligne.</p>
      {session ? (
        <Link to="/app"><button style={btn}>Accéder à mon Dashboard</button></Link>
      ) : (
        <Link to="/signin"><button style={btn}>Se connecter</button></Link>
      )}
    </div>
  );
}
